...
- Superseded: the model is rendered straight down with each surface coloured by world height, the floor and wall tops separate into two clean modes, and the result becomes a 411x231 walkability bitmask in the game's world frame. Art and collision therefore cannot drift apart. Authored prop rectangles still stack on top for tables, cabinets and crates.
- Every station shows its own console art: each of the nine assignments has its real panel (light switches, wiring, nav stabiliser, wifi, garbage chute, power divert, engine align, fuel, weapons), and the meeting button, cameras, admin card slide, vents and sabotage repair points each have theirs. Markers sit on a dark backing disc so they stay readable over the detailed deck.
